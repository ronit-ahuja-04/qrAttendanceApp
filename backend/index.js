...
app.get('/api/faculty/students-by-division', (req, res) => {
  const { division } = req.query;
  if (!division) return res.status(400).json({ error: 'Division is required' });

  db.all(`SELECT id, name, rollNo, deviceId FROM users WHERE role = 'student' AND division = ? ORDER BY rollNo ASC`, [division.toUpperCase()], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/faculty/unbind-division', (req, res) => {
  const { division } = req.body;
  if (!division) return res.status(400).json({ error: 'Division is required' });

  db.run(`UPDATE users SET deviceId = NULL WHERE role = 'student' AND division = ?`, [division.toUpperCase()], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    
    // We could notify all students in the class, but for simplicity, we just return success
    // A future enhancement could loop over db.all and notify each.
    res.json({ message: `Successfully unbound all devices in ${division.toUpperCase()} (${this.changes} students updated)` });
  });
});
