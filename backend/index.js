...
app.get('/api/admin/pilot-cleanup', (req, res) => {
  ...
  const queries = [
    ...
    `UPDATE users SET email = 'pooja.shetty@ves.ac.in' WHERE email = 'prof.ps@ves.ac.in'`,
    `UPDATE users SET email = 'manoj.sabnis@ves.ac.in' WHERE email = 'prof.ms@ves.ac.in'`,
    `UPDATE users SET email = 'pooja.nagdev@ves.ac.in' WHERE email = 'prof.pn@ves.ac.in'`,
    `UPDATE users SET email = 'shanta.sondur@ves.ac.in' WHERE email = 'prof.sso@ves.ac.in'`,
    `UPDATE users SET email = 'charusheela.nehete@ves.ac.in' WHERE email = 'prof.cn@ves.ac.in'`,
    `UPDATE users SET email = 'swapnil.yadav@ves.ac.in' WHERE email = 'prof.sy@ves.ac.in'`,
    
    // 2. Add Nayan Chavan as a proxy faculty
    ...
    `DELETE FROM users WHERE role = 'student' AND (division != 'D15A' OR division IS NULL)`,
    ...
    `DELETE FROM timetable_slots WHERE batchTarget NOT LIKE '%D15A%'`,

    // 6. Wipe unused faculty (except Nayan Chavan)
    `DELETE FROM users 
     WHERE role = 'faculty' 
  ...
  for (const sql of queries) {
    db.run(sql, (err) => {
      ...
      if (completed === queries.length) {
        if (hasError) {
          return res.status(500).json({ error: 'Errors occurred during cleanup. Check server logs.' });
        } else {
          return res.json({ message: 'Pilot Cleanup completed successfully on production database.' });
        }
      }
    });
  }
