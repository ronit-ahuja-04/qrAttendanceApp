  ...
  void _connectWithRetry(String userId) async {
    ...
    try {
      ...
      if (response.statusCode == 401) {
        print('SSE 401 Unauthorized: Triggering auto-logout');
        triggerAutoLogout();
        return;
      }
      ...
      void resetWatchdog() {
        ...
        watchdogTimer = Timer(const Duration(seconds: 25), () {
          print('SSE Watchdog Timeout: No ping received for 25s. Forcing reconnect...');
          _sseSubscription?.cancel();
          _connectWithRetry(userId);
        });
      }

      // Initial watchdog start
      resetWatchdog();

      _sseSubscription = response.stream.transform(utf8.decoder).listen((data) {
        for (final line in data.split('\n')) {
          if (line.startsWith('data: ')) {
            resetWatchdog(); // Reset watchdog on ANY data received
